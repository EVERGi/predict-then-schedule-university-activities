...
def random_solution_times(problem:Problem):
    ...
    sol_time_recur = [random.choice(possible_start[:-i["duration"]]) for i in problem.recur]
    sol_time_once_off = [random.choice(possible_start[:-i["duration"]]) for i in problem.once_off]
...
def random_solution_battery(problem: Problem):
    ...
    batt_sched = [[random.randint(0, 3) for _ in problem.datetimes] for _ in range(num_batteries)]
    return batt_sched
...
def create_feasible_schedule(problem: Problem, sol_recur, sol_once_off, time_recur=None, time_once_off=None):
    ...
    small_slots = [[[[9*4, 17*4]] for _ in range(len(small_rooms))] for _ in range(5)]
    #small_slots = [[[[9*4, 17*4]]]*len(small_rooms)]*5

    large_slots = [[[[9*4, 17*4]] for _ in range(len(large_rooms))] for _ in range(5)]
    #print(small_slots)
    #print(large_slots)
    ...
    for day_to_schedule in range(5):
        ...
        small_slots[day_to_schedule], large_slots[day_to_schedule], sol_small, sol_large = fill_in_schedule(small_slots[day_to_schedule], large_slots[day_to_schedule], activities_to_schedule, day_to_schedule, True, times_to_schedule)
    ...
    times_to_schedule = [time_once_off[id]+day_of_week*24*4-11*4 for id, day_of_week in enumerate(sol_once_off)]
    times_to_schedule = list()
    activities_to_schedule = list()
...
def fill_in_schedule(small_slots, large_slots, activities, day, recur, times_to_schedule):
    ...
    for ind, activity in enumerate(activities):
        new_act_to_sort = {"id": activity["id"], "num_rooms": activity["num_rooms"],
                           "duration": activity["duration"], "time": times_to_schedule[ind]}
        new_act_to_sort["size"] = new_act_to_sort["num_rooms"] * new_act_to_sort["duration"]
    ...
    sorted_activities_small = sorted(unsorted_activities_small, key=lambda k: k['size'])[::-1]
    sorted_activities_large = sorted(unsorted_activities_large, key=lambda k: k['size'])[::-1]
...
def fit_one_activity_with_time(schedule, activity, day, recur):
    ...
    for ind, rooms in enumerate(rooms_to_store):
        ...
        if start != act_start and end != act_end:
            schedule[rooms][avail_ind:avail_ind+1] = [[start, act_start], [act_end, end]]
    ...
    # print(schedule)
    act_sol = {"id": activity["id"], "start_time": act_start, "end_time": act_end, "rooms_index": rooms_to_store,
               "day": day, "recur": recur, "keep": True}
                    ...
                    if time_range[0] == 9*4:
                        ...
                        if month_day == len(problem.all_days)-1:
                            if time_range[0] < 11*4:
                                if time_range[1] <= 11*4:
                                    new_slot = [end_last_day+time_range[0], end_last_day+time_range[1]]
                                    month_slots[0][room].append(new_slot)
                                else:
                                    new_slot = [end_last_day+time_range[0], end_last_day+11*4]
                                    month_slots[0][room].append(new_slot)
                            else:
                                pass
                        else:
...
def remove_once_off(problem: Problem):
    ...
    once_off_sols = [i for i in problem.sol_small if not i["recur"]]+[i for i in problem.sol_large if not i["recur"]]
    ...
    while min_val < 0 and min_id != -1:
        ...
        list_value = [0 if sol_sels[i] in all_prec or zer_val == 0 else None for i, zer_val in enumerate(list_value)]
        list_indiv_value = [0 if sol_sels[i] in all_prec or list_value[i] == 0 else old_val for i, old_val in enumerate(list_indiv_value)]
