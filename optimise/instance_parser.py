...
class Problem():
    ...
    def parse_instance(self):
        ...
        with open(self.filepath) as csvfile:
            ...
            for row in spamreader:
                row = [row[0]]+[int_or_float(i) if i not in ["S", "L"] else i for i in row[1:]]
                if row[0] == "ppoi":
    ...
    def create_sol_string(self):
        ...
        sol_string += "ppoi "+str(self.ppoi["num_build"])+" "+str(self.ppoi["num_solar"])+" "+\
                      str(self.ppoi["num_batt"])+" "+str(self.ppoi["num_recur"])+" "+str(self.ppoi["num_once_off"])+ "\n"

        num_once_off = count_keep(self.sol_small+self.sol_large)

        sol_string += "sched "+str(self.ppoi["num_recur"])+" "+str(num_once_off)+"\n"
        ...
        for sol in self.sol_large:
            if sol["recur"]:
                ...
                if sol["keep"]:
                    strings_once_off[sol["id"]] = self.activity_sol_to_string(sol, False)
        ...
        for batt in self.sol_batt:
            sol_string += "\n"
            sol_string += "c "+str(batt["id"])+" "+str(batt["time"])+" "+str(batt["value"])
        return sol_string
    ...
    def activity_sol_to_string(self, sol, small):
        ...
        line = start_line+str(sol["id"])+" "+time+" "+num_rooms+" "+build_ids
    ...
    def assign_rooms(self, new_sol):
        ...
        if new_sol["recur"]:
            new_start_time = self.monday_index+new_sol["day"]*24*4+new_sol["start_time"]
        else:
    ...
    def get_objective_function(self):
        ...
        load_cost = sum([self.prices[i]*value*0.25/1000 for i, value in enumerate(final_load)])
        ...
        for sol in solutions:
            if not sol["recur"]:
                ...
                if start_dt.weekday() < 5 and 9 <= start_dt.hour <= 16 and end_dt.weekday() < 5 and \
                        (9 <= end_dt.hour <= 16 or (end_dt.hour == 17 and end_dt.minute == 0)):
    ...
    def plot_activities(self, save_fig = False):
        solutions = self.sol_small+self.sol_large
        visualise_activities(solutions, self.base_load, self.prices, self.recur, self.once_off, self.monday_index, 
        self.sol_batt, self.batt, save_fig)
        ...
        if node["num_preccedences"] == 0:
            ...
            for node_j in tree_objects:
                if node["id"] in node_j["precedences"]:
                    nodes_below += [node_j]
            levels_above = 0
            levels_after = recur_levels_below(node_list, nodes_below, levels_above, tree_objects)
            node_list[node["id"]] = {"id": node["id"], "levels_above": levels_above, "levels_after": levels_after, "nodes_below": nodes_below,
            ...
            if node_list[node["id"]] is None:
                ...
                if node_list[node["id"]]["levels_above"] <= old_levels_above:
                    ...
                    for node_j in tree_objects:
                        if node["id"] in node_j["precedences"]:
                            nodes_below += [node_j]
                    recur_levels_below(node_list, nodes_below, node_list[node["id"]]["levels_above"], tree_objects)
