...
from create_solutions import (
    create_feasible_schedule,
    get_objective_function,
    sol_tree_push,
    create_feasible_schedule_batt,
    remove_once_off,
)
...
class MonashSchedule:
    def __init__(self, filepath, start, end, info):
        ...
        if version == 1:
            self.bnds_recur = [
                len(i["days_allowed"]) - epsilon for i in self.tree_recur
            ]
            self.bnds_once_off = [
                len(i["days_allowed"]) - epsilon for i in self.tree_once_off
            ]

            self.bnds_time_recur = [
                (17 - 9) * 4 - i["duration"] - epsilon for i in self.prob.recur
            ]
            self.bnds_time_once_off = [24 * 4 - epsilon for _ in self.prob.once_off]
        elif version == 2:
            self.bnds_recur = [
                len(i["days_allowed"]) * (17 - 9) * 4 - epsilon for i in self.tree_recur
            ]
            self.bnds_once_off = [
                len(self.prob.datetimes) - epsilon for _ in self.tree_once_off
            ]
        ...
        if phase == 1:
            self.instance = (
                self.prob.filepath.split("/")[-1]
                .replace("phase1_instance_", "")
                .replace(".txt", "")
            )
    ...
    def fitness(self, x):
        ...
        if version == 1:
            sol_recur = [
                self.prob.tree_recur[ind]["days_allowed"][choice]
                for ind, choice in enumerate(sol[: len(self.bnds_recur)])
            ]
            days_limit = len(self.bnds_recur + self.bnds_once_off)
            sol_once_off = [
                self.prob.tree_once_off[ind]["days_allowed"][choice]
                for ind, choice in enumerate(sol[len(self.bnds_recur) : days_limit])
            ]

            # print(self.prob.num_weekdays)
            sol_recur = sol_tree_push(self.tree_recur, sol_recur, 5)
            sol_once_off = sol_tree_push(
                self.tree_once_off, sol_once_off, self.prob.num_all_days
            )
            ...
            sol_recur = [
                self.prob.tree_recur[ind]["days_allowed"][time // ((17 - 9) * 4)]
                for ind, time in enumerate(sol[: len(self.bnds_recur)])
            ]
            days_limit = len(self.bnds_recur + self.bnds_once_off)
            sol_once_off = [
                (time + 11 * 4) // (24 * 4)
                for ind, time in enumerate(sol[len(self.bnds_recur) : days_limit])
            ]
            ...
            end_sol_time_recur = days_limit + len(self.bnds_time_recur)
            sol_time_recur = [
                9 * 4 + i % ((17 - 9) * 4) for i in sol[: len(self.bnds_recur)]
            ]
        ...
        try:
            create_feasible_schedule(
                self.prob, sol_recur, sol_once_off, sol_time_recur, sol_time_once_off
            )
        except TypeError:
            self.reset()
            print(200000)
...
def do_optimisation(inst_num, pop_num):
    while True:
        ...
        instances = ["small_" + str(i) for i in range(5)] + [
            "large_" + str(i) for i in range(5)
        ]

        if phase == 1:
            instance_dir = "instances/"
            instance_path = (
                instance_dir + "phase1_instance_" + instances[inst_num] + ".txt"
            )
        ...
        if phase == 2:
            ...
            instance_path = (
                instance_dir + "phase2_instance_" + instances[inst_num] + ".txt"
            )
            start = datetime.datetime(2020, 11, 1)
            end = datetime.datetime(2020, 12, 1)

        monash = MonashSchedule(
            instance_path, start, end, instances[inst_num] + ",cmaes," + str(pop_num)
        )
